...
//			System.out.println("SORTED COUNT" + sorted);
//			System.out.println("UNSORTED COUNT" + unsorted);
		}
	...
	public static final int SINGLE_DUPLICATE_NUMBERS = 3; // minimum number of numbers on the list that will appear
															// twice (have a "single duplicate"
	public static final int DOUBLE_DUPLICATE_NUMBERS = 2; // minimum number of numbers on the list that will appear
	...
	public static List<Integer> findDuplicatesLinear(List<Integer> numbers) {
		...
		for (int i = 0; i < LIST_SIZE; i++) {				//1
			...
			if (!duplicateList.contains(numbers.get(i))) {	//n
				duplicateList.add(numbers.get(i));			//1
		...
		for(int i = 0; i < LIST_SIZE; i++) {				//1
			if(duplicateList.contains(numbers.get(i))) {	//n
				numbers.remove(duplicateList.get(i));		//n
			}
		}
...
//			int test = (numbers.get(i) % randomRange);
//			
//			test = test + randomRange;
//			
//		}
//		
//		for(int i = 0; i < LIST_SIZE;i++) {
...
//			test = test + randomRange;
//			if(test > randomRange) {
//				duplicateList.add(test % randomRange);
//			}
//		}
		return numbers;
	}
